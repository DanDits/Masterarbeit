...
def galerkin_approximation(trial, max_poly_degree, domain, grid_size, start_time, stop_time, delta_time, wave_weight,
    ...
    wave_speeds, matrix_s = calculate_wave_speed_transform(trial, basis, poly_count, chaos)
    ...
    betas, matrix_r = calculate_transformed_betas(trial, basis, poly_count, chaos, matrix_s, xs)
    ...
    return xs, xs_mesh, calculate_expectancy(splitting, stop_time, delta_time)


def get_evaluated_poly(basis, i, chaos):
    rule = chaos.quadrature_rule  # type: QuadratureRule
    quadrature_nodes = rule.get_nodes()
    return np.apply_along_axis(basis[i], 1, quadrature_nodes)


def get_evaluated_polys(basis, ik, chaos):
    return get_evaluated_poly(basis, ik[0], chaos) * get_evaluated_poly(basis, ik[1], chaos)


def calculate_wave_speed_transform(trial, basis, poly_count, chaos):
    def alpha_expectancy_func(nodes_matrix, i, k):
        ...
        res2 = get_evaluated_polys(basis, (i, k), chaos)
        return res1 * res2
...
def calculate_transformed_betas(trial, basis, poly_count, chaos, transform_s, xs):
    ...
    betas = diag.T
    print("Betasshape=", betas.shape, "R shape=", transform_r.shape)
...
def get_projection_coefficients(function, project_trial, basis, chaos):
    ...
    def integrate_func(nodes_matrix, i):
        ...
        res2 = get_evaluated_poly(basis, i, chaos)
        return res1 * res2
...
class MultiLinhypSolver(SolverConfig):
    ...
    def init_solver(self, t0, u0s, u0ts):
        print(len(u0s), len(self.configs))
        assert len(u0s) == len(self.configs)
        assert len(u0ts) == len(self.configs)
        self.init_initial_values(t0, u0s, u0ts)
...
def calculate_expectancy(splitting, stop_time, delta_time):
    ...
    expectancy = np.array([coeffs[0] for coeffs in grid_coeffs])
    return expectancy
