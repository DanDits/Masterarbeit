...
\todo[inline]{Erkläre Löser, dazu für wave teil: periodische RB-> Fourier, für linhyp: explizite Lösung; Wie Gleichung in gleichung 1. Ordnung erst umschreiben (weight w); wieso nur stabil für w=1?; zeige verschiedene Kombinationen von RStrang, FStrang,... und zeige Konvergenzordnung p=1 und p=2}
...
\begin{pmatrix}u\\v\end{pmatrix}(t,x)
\end{align}
Doch bevor wir dies als Grundlage für ein Splitting verwenden, müssen wir den Operator $\Laplace$ genauer untersuchen.
\subsubsection*{Fourier Pseudo-Spectral Methode für Laplace Operator}
Wir betrachten das Teilproblem der Wellengleichung mit periodischen Randbedingungen auf $(-\pi,\pi)^d$ der Dimension $d\in\N$ und dem Quadrat der Wellengeschwindigkeit $c^2=\alpha>0$
\begin{equation}
...
\end{equation}
für gewisse Koeffizienten $\fourier{u}_k(t)=\int_{-\pi}^\pi u(t,x)e^{ikx}dx\in\Complex$. Setzen wir die Fourier-Approximation als exakte Lösung der Wellengleichung (\ref{eqn:waveperiodic}) ein, so erhalten wir 
\[\sum_{k=-N}^N\fourier{u}_k^{\prime\prime}(t)e^{ikx}=c^2\sum_{k=-N}^N-k^2\fourier{u}_k(t)e^{ikx}\]
Da die $e^{ik\cdot}$ eine Basis bilden müssen wir also für $k=-N,\dots,N$ die gewöhnliche Differentialgleichung
\begin{align*}
\fourier{u}_k^{\prime\prime}(t)&=-c^2k^2\fourier{u}_k(t)\\
\fourier{u}_k(0)&=\fourier{g}_k\\
\fourier{u}_k^\prime &=\fourier{h}_k
\end{align*}
...
\begin{align*}
u(t,x_j)&=\mathcal{F}^{-1}\left[\mathcal{F}g\odot \big(\cos(c|k|t)\big)_{k=-N}^N + \mathcal{F}h\odot \big(\sinc(c|k|t)\big)_{k=-N}^N\right]_j\\
...
\end{align*}
Dabei bedeutet $\odot$ die punktweise Multiplikation der Vektoren. Man beachte, dass die Sortierung der Fourier-Koeffizienten in verschiedenen Programmiersprachen verschiedenen Notationen folgt und entsprechend die Indizierung angepasst werden muss. Beispielsweise gilt für die Python3 Bibliothek \emph{numpy} durch die entsprechende Funktion \emph{numpy.fft.fftfreq(8)*8} die Konvention
\[[ 0,  1,  2,  3, -4, -3, -2, -1]\]
wobei Matlab die ähnliche Konvention
\[[ 0,  1,  2,  3, 4, -3, -2, -1]\]
verwendet.\\
Obige Erläuterungen waren für eine räumliche Dimension $d=1$. Will man nun den Ansatz auf mehrere Dimensionen erweitern, so erhält man ein sehr ähnliches Ergebnis.\todo[inline]{Schreibe hin dass vorheriges Ergebnis mit betrag k ersetzt durch sqrt(k1hoch2+...+kdhoch2) gilt, alles andere wie Indizierung und FFT enstprechend auch mehrdimensional.}
