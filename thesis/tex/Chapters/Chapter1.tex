...
\begin{algorithm}[ht]
    ...
    \begin{algorithmic}[1] % The number tells where the line numbering should start
        \Function{Strang\_Step\_KGG}{$N,u_0,v_0,\alpha,\beta,w,t$} 
            \State $x_N\gets (-\pi+\frac{2\pi j}{N})_{j=0,\dots,N-1}^T$
            \State $g,h\gets u_0(x_N), v_0(x_N)$
            \State $g,h\gets \text{First\_Part\_Solver}(N, g, h, \alpha, w, \frac{t}{2})$
            \State $g,h\gets \text{Second\_Part\_Solver}(x_N, g, h, \beta, 1-w, t)$
            \State $u_1,v_1\gets \text{First\_Part\_Solver}(N, g, h, \alpha, w, \frac{t}{2})$
            \State \textbf{return} $u_1, v_1$\Comment{Approximation an $u(t,x_N)$ und $\dt{u}(t,x_N)$}
        \EndFunction
        
        \Function{First\_Part\_Solver}{$N,g,h,\alpha,w,t$} 
        	...
        	\State $c\gets \big(\cos(t\sqrt{\alpha w}|k|)\big)_{k\in\mathcal{I}_N}$
        	\State $s_1\gets \big(\sinc(t\sqrt{\alpha w}|k|)tw\big)_{k\in\mathcal{I}_N}$
        	\State $s_2\gets \big(-\sinc(t\sqrt{\alpha w}|k|)\alpha|k|^2t\big)_{k\in\mathcal{I}_N}$
        ...
        \EndFunction
        \Function{Second\_Part\_Solver}{$x_N,g,h,\beta,\tilde{w},t$} 
        	\State $\beta_N\gets\beta(x_N)$
        	\State $c\gets \cos(t\sqrt{\beta_N}\sqrt{\tilde{w}})$
        	\State $s_1\gets \sinc(t\sqrt{\beta_N}\sqrt{\tilde{w}})\tilde{w}t$
        	\State $s_2\gets -\sinc(t\sqrt{\beta_N}\sqrt{\tilde{w}})\beta_Nt$
...
v_0(x)&\equiv 0\\
\end{align*}
eine Lösung der KGG (\ref{kgg}) mit $\alpha=c-2$ und $\beta(x)=d^2+(c-2)\left(\frac{\sin(x)}{\sin(x)+c}+\frac{2\cos^2(x)}{(\sin(x)+c)^2}\right)$. Wir lösen nun die KGG mithilfe verschiedener Splitting Varianten und beobachten in Abbildung \ref{fig:splitting_convergence} eine Konvergenz von Ordnung 2 für das Strang-Splitting und Konvergenz von Ordnung 1 für das Lie-Trotter-Splitting. Die schnelle Variante des Strang-Splittings (FastStrang), bei dem die Halbschritte kombiniert werden, hat dabei exakt den gleichen Fehler wie das normale Strang-Splitting. Die Variante, wo die Reihenfolge der beiden Aufteilungen der Gleichung vertauscht wurde (ReversedStrang) unterscheidet sich nur minimal von der normalen Variante.\\
Am deutlichsten sticht aber etwas anderes hervor: Wird die Zeitschrittweite $\tau$ zu groß, d.h. ist $1.227\tau>\frac{2\pi}{N}=h$, so ist das Verfahren instabil.
\begin{figure}[!htb]
...
\end{figure}
Dabei spielt der Gewichtsfaktor $w$ eine große Rolle. Wäre $w=0$, so entspricht der Löser des ersten Teil des Splittings der Vorschrift
\begin{align*}
