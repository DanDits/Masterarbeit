...
\subsubsection*{Fourier Pseudo-Spectral Methode für Laplace Operator}
Wir betrachten das Teilproblem der Wellengleichung mit periodischen Randbedingungen auf $(-\pi,\pi)^d$ der Dimension $d\in\N$ und dem Quadrat der Wellengeschwindigkeit $c^2>0$
\begin{equation}
\label{eqn:waveperiodic}
\begin{split}
\dtt{u}(t,x)&=c^2 \Laplace u(t,x), \quad t>0, x\in\Torus^d\\
u(0,x)&=g(x), \quad x\in\Torus^d\\
\dt{u}(0,x)&=h(x),\quad x\in\Torus^d\\
u, \dt{u}&\text{ periodisch in }x
\end{split}
\end{equation}
Man beachte, dass dann insbesondere $g$ und $h$ periodisch in $x$ sein müssen.
Im Folgenden zeigen wir eine Darstellung der exakten Lösung von (\ref{eqn:waveperiodic}) für $d=1$.\\
Es gilt für die Fourier-Approximation von $u$ für jedes $t>0$ und $N\in\N$
\begin{equation}
u(t,x)\approx \sum_{k=-N}^N\fourier{u}_k(t)e^{ikx}, \quad x\in\Torus^d
\end{equation}
für gewisse Koeffizienten $\fourier{u}_k(t)=\frac{1}{2\pi}\int_{-\pi}^\pi u(t,x)e^{-ikx}dx\in\Complex$. Setzen wir die Fourier-Approximation als exakte Lösung der Wellengleichung (\ref{eqn:waveperiodic}) ein, so erhalten wir 
\[\sum_{k=-N}^N\fourier{u}_k^{\prime\prime}(t)e^{ikx}=c^2\sum_{k=-N}^N-k^2\fourier{u}_k(t)e^{ikx}\]
Da die $e^{ik\cdot}$ eine Basis des Fourier-Raums bilden, müssen wir also für $k=-N,\dots,N$ die gewöhnliche Differentialgleichung
\begin{equation}
\label{eqn:fourier_dgl}
\begin{split}
\fourier{u}_k^{\prime\prime}(t)&=-c^2k^2\fourier{u}_k(t)\\
\fourier{u}_k(0)&=\fourier{g}_k\\
\fourier{u}_k^\prime(0) &=\fourier{h}_k
\end{split}
\end{equation}
lösen, wobei $\fourier{g}_k$ bzw. $\fourier{h}_k$ der $k$-te Fourier-Koeffizient von $g$ bzw. $h$ ist. Aus der charakteristischen Gleichung folgt für zwei Konstanten $c_{1,k},c_{2,k}$ sofort die allgemeine Lösung 
\[\fourier{u}_k(t)=c_{1,k}\cos(c|k|t)+c_{2,k}\sin(c|k|t)\]
und unter Berücksichtigung der Anfangswerte
\begin{align*}
\fourier{u}_k(t)&=\fourier{g}_k \cos(c|k|t)+\fourier{h}_kt \sinc(c|k|t)\\
\fourier{u}^\prime_k(t)&=-c|k|\fourier{g}_k\sin(c|k|t)+\fourier{h}_k\cos(c|k|t)\\
\end{align*}
Diese Darstellung ist insbesondere auch für $k=0$ gültig, wo der Fourierkoeffizient linear in $t$ wächst.\\
Formalisieren wir nun die diskrete Fouriertransformation, die wir verwenden möchten, um eine Approximation an die Fourier-Koeffizienten $\fourier{g}_k$ bzw. $\fourier{h}_k$ zu erhalten.
\begin{mathdef}[Diskrete Fourier-Transformation]
Die lineare Abbildung $\mathcal{F}\colon\Complex^N\to\Complex^N$ definiert durch
\[\mathcal{F}x=\fourier{x},\qquad\text{mit}\quad \fourier{x}_k=\sum_{j=0}^{N-1}e^{-2\pi i\frac{jk}{N}}x_j\]
heißt diskrete Fourier-Transformation.
\end{mathdef}
Es existiert eine schnelle Berechnung der Abbildung in $\mathcal{O}(N\log N)$ Schritten anstelle der direkten Implementierung in $\mathcal{O}(N^2)$ Schritten, bekannt unter dem Namen \emph{Fast Fourier Transform}(fft). Da diese auf der Annahme $2N=2^p$ für ein $p\in\N$ basiert, werden wir in Zukunft nur noch solche Approximationen verwenden. Dies ist essentiell, um eine schnelle Berechnung der Lösung der KGG zu gewährleisten. Es gilt $\mathcal{F}_N\overline{\mathcal{F}}_N=NI$ und somit hat die inverse Fourier-Transformation beinahe die selbe Gestalt wie die Fourier-Transformation selbst und kann ebenso schnell berechnet werden.\\[0.2cm]
Zusammengefasst gilt also für die Lösung von (\ref{eqn:waveperiodic}) an den Stellen $x_j=-\pi+2\pi\frac{j}{2N}\in [-\pi,pi)$ für $j=0,\dots,2N-1$ die Darstellung
\begin{align*}
u(t,x_j)&=\left(\mathcal{F}^{-1}\left[\mathcal{F}g\odot \big(\cos(c|k|t)\big)_{k=-N}^N + \mathcal{F}h\odot \big(\sinc(c|k|t)\big)_{k=-N}^N\right]\right)_j\\
\dt{u}(t,x_j)&=\mathcal{F}^{-1}\left[\mathcal{F}g \odot -c\big(|k|\sin(c|k|t)\big)_{k=-N}^N+\mathcal{F}h\odot \big(\cos(c|k|t)\big)_{k=-N}^N\right]_j
\end{align*}
Dabei ist $\odot$ die punktweise Multiplikation der Vektoren. Man beachte, dass die Sortierung der Fourier-Koeffizienten in verschiedenen Programmiersprachen verschiedenen Notationen folgt und entsprechend die Indizierung angepasst werden muss. Beispielsweise gilt für die Python3 Bibliothek \emph{numpy} durch die entsprechende Funktion \emph{numpy.fft.fftfreq(8)*8} die Konvention
\[[ 0,  1,  2,  3, -4, -3, -2, -1]\]
...
\[u(t,x)\approx\sum_{k_1=-N}^N\fourier{u}_{k_1}(t,x_2)e^{ik_1x_1}=\sum_{k_1=-N}^N\sum_{k_2=-N}^N\fourier{u}_{k_1,k_2}(t)e^{i(k_1x_1+k_xx_2)}\]
und analog zu den gewöhnlichen Differentialgleichungen (\ref{eqn:fourier_dgl}) die gewöhnlichen Differentialgleichungen
\[\fourier{u}^{\prime\prime}_{k_1,k_2}(t)=-c^2(k_1^2+k_2^2)\fourier{u}_{k_1,k_2}(t),\quad k_1,k_2=-N,\dots,N\]
Ersetzt man also in obigen Ausführungen für $d=1$ sämtliche $|k|$ durch \[\norm{k}_2=\sqrt{k_1^2+\dots+k_d^2}\]so erhält man eine Darstellung der Lösung für beliebige Dimensionen $d\in\N$.
\subsubsection*{Method of lines für zweiten Teil des Splittings}
Einen Löser für den zweiten Teil des Splittings für die KGG zu bekommen ist hingegen deutlich einfacher. Wir suchen für $t>0$, $\beta(x)>0$ und eine Konstante $c>0$ die Lösung der Differentialgleichung
\begin{equation}
\begin{split}
\dtt{u}(t,x)&=-c\beta(x)u(t,x),\quad x\in\Torus^d\\
u(0,x)&=g(x),\quad x\in\Torus^d\\
\dt{u}(0,x)&=\frac{h(x)}{c},\quad x\in\Torus^d\\
u, \dt{u}& \text{ periodisch in }x
\end{split}
\end{equation}
Die Einführung der Konstanten $c$ ist dabei hilfreich, um später das Gewicht $w$ des Splittings ohne weitere Schwierigkeit darzustellen.\\
Mithilfe der Darstellung als Gleichung erster Ordnung in der Form
\begin{equation}
\label{eqn:split2_firstorder}
\dt{\begin{pmatrix}u\\v\end{pmatrix}}(t,x)=
\underbrace{
\begin{pmatrix}
0 & cI\\
- \beta(x) & 0\\
...
\begin{pmatrix}u\\v\end{pmatrix}(t,x)
\end{equation}
erhalten wir für festes $x\in\R$ die explizite Lösung mithilfe der Matrixexponentialfunktion
\[\begin{pmatrix}u\\v\end{pmatrix}(t,x)=e^{M(x)t}\begin{pmatrix}g(x)\\h(x)\end{pmatrix}=
\begin{pmatrix}
\cos(\sqrt{c\beta(x)}t) & \sqrt{\frac{c}{\beta(x)}}\sin(\sqrt{c\beta(x)}t)\\
-\sqrt{\frac{\beta(x)}{c}}\sin(\sqrt{c\beta(x)}t) & \cos(\sqrt{c\beta(x)}t)\\
\end{pmatrix}
\begin{pmatrix}g(x)\\h(x)\end{pmatrix}\]
Mithilfe der \emph{method of lines}, d.h. punktweiser Auswertung der Lösung für alle benötigten $x$, bekommt man somit eine numerische Lösung.
\subsubsection*{Splitting für die KGG}
Verwenden wir nun das formale Splitting (\ref{eqn:formalsplit_kgg}) so erhalten wir für ein Gewicht $w\in [0,1]$ die Differentialgleichungen
\begin{equation}
\label{eqn:kgg_split1}
\begin{pmatrix}u\\v\end{pmatrix}(t,x)\\
=\begin{pmatrix} 0 & wI\\ \alpha \Laplace & 0\end{pmatrix} 
\begin{pmatrix}u\\v\end{pmatrix}(t,x)
\end{equation}
und
\begin{equation}
\label{eqn:kgg_split2}
\begin{pmatrix}u\\v\end{pmatrix}(t,x)\\
=\begin{pmatrix} 0 & (1-w)I\\ -\beta(x) & 0\end{pmatrix}
\begin{pmatrix}u\\v\end{pmatrix}(t,x)
\end{equation}
Die Differentialgleichung (\ref{eqn:kgg_split1}) entspricht dabei dem System (\ref{eqn:waveperiodic}), für das wir mithilfe der Fourier Pseudo-Spectral Methode eine numerische Lösung bekommen.
Die Differentialgleichung (\ref{eqn:kgg_split2}) lösen wir mithilfe der Darstellung (\ref{eqn:split2_firstorder}) und zugehöriger Matrixexponentialfunktion.\\
Beachte, dass nur das zweite Einschrittverfahren exakt ist. Für das erste benötigen wir gewisse Anforderungen an die Glattheit von $u$ in $x$. Dabei ist bereits $u$ stetig und stückweise stetig differenzierbar in $x$ ausreichend für die gleichförmige Konvergenz der Fourierapproximation. 
\begin{mathbsp}
