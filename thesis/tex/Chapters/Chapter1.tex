...
\[C_\text{per}^\infty (\Torus^d)\coloneqq \lbrace v\colon \Torus^d\to\R \mid v \text{ beliebig häufig stetig differenzierbar und periodisch in }[-\pi,\pi]\rbrace\]
Definieren wir zu $v\in C^\infty(\Torus^d)$ und $k\in\N_0$ die Sobolev-Norm $\norm{v}_{k,2}$ durch
...
Wir definieren einen vollständigen Unterraum --und somit einen Banachraum-- des Sobolev-Raums $H^k((-\pi,\pi)^d)$ durch
\[H_\text{per}^k(\Torus^d)\coloneqq \text{Abschluss von } C_\text{per}^\infty(\Torus^d) \text{ bezüglich der Norm von } H^k((-\pi,\pi)^d)\]
\end{mathdef}
...
\[\forall r\in R\colon S_\text{Orbit}(\cdot)\colon \R_+\to R,\: t\mapsto S(t)r \text{ ist stetig}\]
...
Es ist $D(A)=H^2_\text{per}(\Torus^d)\times H^1_\text{per}(\Torus^d)\subset R$ dicht in $R$, da bereits $C_\text{per}^\infty\times C_\text{per}^\infty$ dicht in $R$ ist.\\
...
\todo[inline]{Passt das mit der Norm für $B$? Können BRe auch einfach reell sein?}
