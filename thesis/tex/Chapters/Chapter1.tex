...
Man beachte, dass der Name "`Bestapproximation"' sich strikt auf die verwendete Norm bezieht. Punktweise Aussagen lassen sich somit im Allgemeinen nicht treffen. Vergleicht man beispielsweise die Bestapproximation und die Chebyshev Interpolation vom Grad $N=100$ von der Funktion $f(x)=|x-\frac{1}{4}|$ in $[-1,1]$, so ist der maximale Fehler der Bestapproximation zwar geringer, aber tritt an $N+2$ Punkten auf, wohingegen der Fehler der Chebyshev Fehler zu den Rändern hin abnimmt.\todo{Plotte Bild aus Myth4}
\todo[inline]{Zeige Approximationstheorie Ergebnisse: L2 Konvergenz (Satz 3.5 Xiu) er zeigt auch explizit für Legendre, order!=accuracy (siehe code zitat), Fehler für Interpolation/Quadratur mit Gauss erst später dann}