...
Es ist $D(A)=H^2(\Torus^d)\times H^1(\Torus^d)\subset R$ dicht in $R$, da bereits $C^\infty(\Torus^d)\times C^\infty(\Torus^d)\subset D(A)$ dicht in $R$ ist.\\
...
\end{align*}
Wäre $A$ beschränkt, so folgt daraus sofort die Schiefadjungiertheit $A=-A^*$ von $A$. Da in unserem Fall $A$ unbeschränkt ist, müssen wir zusätzlich die Bedingung $D(A)=D(A^*)$ erfüllen. Dazu benutzen wir die Tatsache, dass $A$ schiefadjungiert äquivalent ist zu $iA$ selbstadjungiert. Nach \autocite[Bemerkung 5.6f]{HundSchnau} müssen wir für die Selbstadjungiertheit von $iA$ folgende Bedingungen erfüllen:\\
\begin{itemize}
\item $iA$ symmetrisch
...
Die Abgeschlossenheit von $A$ ist äquivalent zur Abgeschlossenheit von $A$. Sei dazu
\begin{align*}
...
&\le \norm{u_2-u_{2,n}}_{H^1}+\norm{\Laplace(u_1-u_{1,n})}_{L^2}+\norm{u_1-u_{1,n}}+\norm{Au_n-y}\to 0 (n\to\infty)
\end{align*}
ist $Au=y$ und $A$ somit abgeschlossen.\\
