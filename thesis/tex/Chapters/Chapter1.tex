...
\todo[inline]{Späteres Kapitel: Unsere Anwendung auf KGG; WaveSolver und LinhypSolver; Stabilität ohne CFL für $w=1$ (hat das was mit der NichtSymmetrie des Strang Splittings zu tun?}
...
\section{General Polynomial Chaos}
...
\todo[inline]{Einführung in PolyChaos; zeige Konvergenz in L2 der poly expansion und dass besser für entsprechende PolyBasis aber auch funktional für andere mithilfe von experiment}
\subsection{Orthogonale Polynome und Approximationstheorie}
...
\end{eqnarray}
wobei $a_n=\frac{k_n}{k_{n-1}}$, $b_n\in\R$ und $c_n=-\frac{a_n}{a_{n-1}}\cdot \frac{\gamma_n}{\gamma_{n-1}}$.
Dabei ist $k_n\neq 0$ der führende Faktor und $\gamma_n>0$ der Normalisierungsfaktor des Polynoms $Q_n$ . 
\todo[inline]{erkläre/beweise wie man daraus auf Nullstellenberechnung kommt später für Nodes/weights}
\end{maththeorem}
\begin{proof}
...
\[Q_n(x)-a_nxQ_{n-1}(x)=\sum_{m=0}^{n-1}c_mQ_m(x)\]
Für $k\le n-1$ folgt also aus der Orthogonalität bezüglich des Skalarproduktes $\langle\cdot,\cdot\rangle$
\begin{equation*}
\langle Q_n(x)-a_nxQ_{n-1}(x),Q_k(x)\rangle=\sum_{m=0}^{n-1}c_m\langle Q_m(x),Q_k(x)\rangle=c_k\langle Q_k(x),Q_k(x)\rangle=c_k\gamma_k
\end{equation*}
und durch Verwenden von $\langle Q_n(x),Q_k(x)\rangle = 0$ und Umstellen
\[ -a_n\langle Q_{n-1}(x),xQ_k(x)\rangle=c_k\gamma_k\]
Somit folgt für $k<n-2$ wegen $\text{deg}(xQ_k(x))<n-1$ dass $c_k=0$. Damit erfüllen die Polynome eine 3 Term Rekursion
\[Q_n(x)-a_nxQ_{n-1}(x)=c_{n-1}Q_{n-1}(x)+c_{n-2}Q_{n-2}(x)\]
Weiterhin gilt \[c_{n-2}\gamma_{n-2}=-a_n\langle Q_{n-1}(x),xQ_{n-2}(x)\rangle=-a_n\frac{k_{n-2}}{k_{n-1}}\gamma_{n-1}\]
\end{proof}
...
\begin{maththeorem}
\todo[inline]{Using \autocite{GolubWelsch} (and own notes for D) show how to calculate roots and therefore gauss quadrature}
\end{maththeorem}
\todo[inline]{Zeige Approximationstheorie Ergebnisse: Punktweise Konvergenz (gibbs), L2 Konvergenz, order!=accuracy (siehe code zitat)}