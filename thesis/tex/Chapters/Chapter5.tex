...
Die partielle Differentialgleichung (\ref{eqn:galerkin_dgl}) ist ein System mit Zeitableitung zweiter Ordnung und vektorwertig mit Größe $M+1$. Um eine numerische Lösung des Systems zu erhalten, bietet sich wieder ein Splitting-Ansatz an. Zuerst wollen wir das System aber so weit wie möglich entkoppeln. Dazu beobachten wir, dass die Matrix $A$ symmetrisch positiv definit ist,\todo{Warum? Das passt so nicht.} da $\alpha(y)>0$ und somit $a_{mk}>\delta_{mk}$. Wir können $A\in\R^{M+1\times M+1}$ zerlegen in
...
\end{equation}
Diese sind wie schon in Kapitel \ref{chapter:solver_kgg} mithilfe der Matrixexponentialfunktion lösbar. Beachte, dass die Gleichung (\ref{eqn:galerkin_split1}) entkoppelt ist, da $D$ diagonal ist. Der erste Teil des Splittings besteht daher aus dem Lösen von $M+1$ unabhängigen "'Wellengleichungen"' (für $\kappa=1$), wobei die Wellengeschwindigkeiten die Eigenwerte $d_m$ der Matrix $A$ sind.\\
Der zweite Teil des Splittings muss für jedes $x$ im benötigten Gitter extra gelöst werden.