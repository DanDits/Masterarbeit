...
class Splitting:
    def __init__(self, configs, step_fractions, config_initializers, on_end_callback=None):
...
def make_klein_gordon_lie_trotter_reversed_splitting(intervals, grid_points_list, t0, u0, u0t, alpha, beta):
    ...
    init_linhyp_solver(linhyp_config, t0, u0, u0t)
    linhyp_config.start_velocity *= 0.5

    return Splitting([linhyp_config, wave_config], [1., 1.], [init_linhyp_solver, init_wave_solver])
...
def make_klein_gordon_strang_reversed_splitting(intervals, grid_points_list, t0, u0, u0t, alpha, beta):
    ...
    return Splitting([linhyp_config, wave_config, linhyp_config], [0.5, 1., 0.5],
                     [init_linhyp_solver, init_wave_solver, init_linhyp_solver])
...
def make_klein_gordon_fast_strang_splitting(intervals, grid_points_list, t0, u0, u0t, alpha, beta, time_step_size):
    ...
    next_position, next_velocity = Splitting.sub_step(wave_config, wave_config.start_time,
                                                      time_step_size, 0.5)
    init_linhyp_solver(linhyp_config, wave_config.start_time, next_position, next_velocity)
    ...
    init_wave_solver(wave_config, t0 + time_step_size, next_position, next_velocity)
    ...
    splitting = Splitting([wave_config, linhyp_config], [1., 1.], [init_wave_solver, init_linhyp_solver],
