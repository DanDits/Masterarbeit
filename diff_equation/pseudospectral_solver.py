...
def make_wave_config(intervals, grid_points_list, wave_speed):
    assert wave_speed > 0.

    config = SolverConfig(intervals, grid_points_list, pseudospectral_power=2)
    config.wave_speed = wave_speed
    ...
    config.norm2_factors = np.sqrt(-sum(config.pseudospectral_factors_mesh).real)

    # top left corner of norm2_ks contains a zero, temporarily replace it
    config.zero_index = (0,) * len(config.norm2_factors.shape)
    assert config.norm2_factors[config.zero_index] == 0
    return config
...
def init_wave_solver(config, t0, u0, u0t):
    ...
    y0_ = fftn(config.start_position)  # starting condition in fourier space and evaluated at grid
    y0t_ = fftn(config.start_velocity)

    # calculate factors c1_ and c2_
    c1_ = y0_
    if abs(y0t_[config.zero_index]) > config.norm2_factors.size * 1e-9:  # as the fourier coefficients are unscaled
        ...
        print("Warning! Start velocity for wave solver not possible, solution will be incorrect: "
              + "0th fourier coefficient not zero but", y0t_[config.zero_index], "for size", config.norm2_factors.size)
    # this makes sure that c2_[zero_index] is 0 and no warning is triggered
    config.norm2_factors[config.zero_index] = np.inf
    c2_ = y0t_ / config.norm2_factors
